...
class PhotoRepository(private val context: Context) {
    ...
    }.flowOn(Dispatchers.Default) // fusionar miles de fotos con sus metadatos es trabajo
    // suficientemente pesado como para no hacerlo en el hilo principal; antes se hacía
    // ahí y era, junto al doble escaneo, la causa del "no responde" al abrir la app.

    /**
     * Lee el MediaStore real del dispositivo. Llamar al iniciar y tras cambios de permisos.
     * Muestra las fotos EN TANDAS a medida que las va leyendo (como hacen el resto de
     * apps de galería), en vez de esperar a recorrer toda la biblioteca antes de mostrar
     * algo — con muchas fotos, esa espera era lo que hacía sentir la app "colgada".
     */
        ...
        context.contentResolver.query(
            ...
            while (cursor.moveToNext()) {
                ...
                if (pending.size >= batchSize) {
                    _mediaImages.value = _mediaImages.value + pending
                    pending.clear()
                }
            }
        }
        if (pending.isNotEmpty()) _mediaImages.value = _mediaImages.value + pending

        // Crea metadatos por defecto para fotos nuevas. Corre al final, después de que
        // todo ya está visible: no hace falta esperarlo para poder ver/usar las fotos.
        metaDao.insertDefaults(allIds.map { PhotoMeta(mediaStoreId = it) })
    }
